...
// Sitemap status dashboard - Plain text view (no Vite interference)
router.get('/api/sitemap-dashboard', async (req, res) => {
  try {
    const categories = await db.select().from(serviceCategories);
    const allServices = await db.select().from(services);
    const allFeatures = await db.select().from(features);
...
✓ Service Categories: ${stats.serviceCategories}
✓ Service Details: ${stats.serviceDetails}
✓ Feature Details: ${stats.featureDetails}
✓ Project Pages: ${stats.projectPages}
...
🔗 JSON API: /api/sitemap/status
🔗 XML Sitemap: /sitemap.xml
🔗 HTML Sitemap: /sitemap

=====================================

Your IeNet platform is fully operational with ${stats.totalPages} pages covering the complete IT services hierarchy. All content is properly seeded and accessible for users and search engines.
...
=====================================
    ...
    console.error('Error getting sitemap dashboard:', error);
    res.status(500).json({ error: 'Failed to get sitemap dashboard' });
  }
});
